...
func modelPriceNotConfiguredError(modelName string, userId int) error {
	if model.IsAdmin(userId) {
		return fmt.Errorf(
			"model %s price is not configured; enable self-use mode in System Settings -> Operation Settings, or configure the model price in System Settings -> Group & Model Pricing",
			modelName,
		)
	}
	return fmt.Errorf(
		"model %s has not been priced by the administrator; please contact the site administrator to enable this model",
		modelName,
	)
}
